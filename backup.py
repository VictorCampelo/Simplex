...
def iteracao_w(tableaux, W, l, c, lPivo, cPivo):
...
def primeira_fase(tableaux, W, l, c, tipo_restricao):
...
def imprime_tableaux(l, c, tableaux, tipo_restricao):
    ...
    k2 = 0
    for i in range(0, (c - 2)-1):   #2.0 - c-2 pois a quantidade de X � a quantidade de colunas menos a linha Z e a linha b
        k = 0  #2.0 - k indica se o x est� ou n�o na base
        print("X%d = ", i + 1)
        for j in range(0, cont-1):#2.0 - percorre o vetor e busca se o �ndice do x em quest�o est� na base
...
def imprime_problema(op, l, c, Z, restricoes, tipo_restricao):
    cont = 0
    #Imprime o problema
    print("O problema e:\n")
    ...
    print("\n\nSujeito a:\n\n")
    for i in range(0, l-1):
        for j in range(0, (c - 1)-1):   #Vai at� o utimo X
            if j == 0:
...
def imprime_na_forma_padrao(l, c, Z, padrao, tipo_restricao, q_var, op):
    cont = 0
    ...
    if op == 1:
        ...
        print("\n- Z ")
    for j in range(0, q_var-1):
        if Z[j] >= 0:
            print("+ ")
        else:
            print("- ")
        print("%.0lfX%d ", (modulo(Z[j]), j + 1))
    for i in range(q_var + 1, cont < l):
        if tipo_restricao[cont] == 1:
            print("+ 0X%d ", (i))
        else:
            print("+ 0X%d ", (i))
    ...
    for i in range(0, l-1):
        ...
        for j in range(0, c-1):
            if(j == c - 1):   #Verifica se vai ler um coeficiente de X ou a igualdade da restri��o
                ...
                print("1. <=\n")
                print("2. >=\n? ")
                tipo_restricao.append(int(input()))
...
def verificar_otimo(tableaux, c, l, cPivo):   #1.6 - O cabe�alho da fun��o foi alterado, h� a necessidade de receber o numero de linhas do tableaux e a coluna Piv�.
    ...
    if(cPivo != 0):   #1.8.1 - Precisei fazer isso pra corrigir um bug, 0 como valor inicial do indice de variable_in, no ultimo tableaux, n�o tem variaveis pra entrar a fun��o vai jogar um lixo em cPivo, quando tu tentar ler ela aqui o programa vai parar de funcionar, n�o existe a coluna "lixo"
        for i in range(0, l-1): #1.6 - para percorrer toda a coluna piv�
...
def variable_out(tableaux, ind, l, c):
...
def resultado_tableaux(tableaux, l, c, tipo_restricao):
    isOtimo, _in, out, i, aux, j
...
def iteracao_tableaux(tableaux, l, c, lPivo, cPivo):
    i, j
    aux
    aux = tableaux[lPivo][cPivo]
    for i in range(0, c-1): #1.5 - para o c�lculo da nova linha piv�
    ...
    for i in range(0, l-1):   #1.5 - inicia o percurso pela primeira linha do tableaux
        if(i != lPivo): #1.5 - s� prossigo com os c�lculos se a linha atual n�o for a linha piv�, ela j� foi calculada antes
            for j in range(0, c-1):   #1.5 - caso n�o seja a LP, toda a linha
                if j == 0:
    ...
    for i in range(0, (l-1)-1):
        for j in range(0, (c - 1)-1):
            if(j < q_var):  #Ate o tamanho da matriz restricoes apenas copia os coeficientes de uma pra outra
                padrao[i].append(restricoes[i][j])
                print(restricoes[i][j])
            else:
